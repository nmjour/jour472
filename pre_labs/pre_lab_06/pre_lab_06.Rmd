...
**Task** Load the four CSV population estimate files. Do they have the same column structure? **Answer** Somewhat. Each data frame shows the year, county, amount of voters per party, and total amount of voters. However the 2022 and 2022 data sets have some of the columns are arranged in a different order compared to the 2016 and 2018 data sets. The 2022 data set also has the "Total" column in sentence case, instead of uppercase like the previous years' data sets.
...
# rename total column to uppercase
  rename(TOTAL = Total)
```
...
**Task** Run the following code to filter on Frederick County's registration data and look at the results. What stands out to you about the changes in party registration over time? **Answer** The Democratic party in Frederick has been growing over the last six years, reaching almost 8,000 voter registrants in 2022. The Republican party has also grown a bit, but not as much compared to the Democratic party. There has also been considerable growth in voters registering with smaller political parties.
...
**Task** Load the Maryland demographic data and examine it. What kind of information does it contain? **Answer** The demographic data splits up county population by race, as well as the adult population over 18 years old.
...
**Task** Using mutate, let's find the county with the highest 18+ Black population as a percentage of all population 18 and over and also the percentage of Democrats in that county. Put your answers below. **Answer** The county with the highest 18+ Black population is Prince George's County (at 60.9%), where 77% of registered voters are Democrats. Another interesting finding is the rural Eastern Shore county of Somerset, where Black residents make up 39% of the 18+ population, and 39.5% of registered voters there are Democrats. It's likely that the Black residents in Somerset are also Democrats.
...
**Task** Now let's find the county with the highest 18+ Asian population as a percentage of all population 18 and over and also the percentage of Democrats in that county. How do these results compare to the previous task? Put your answers below. **Answer** The county with the highest 18+ Asian population is Howard (at 19.37%), where 52.6% of registered voters are Democrats. Overall, there are less Asian residents in Maryland compared to Black residents, and a majority of both populations reside in different counties. Asian residents also tend to reside in counties that have variable political affiliations (Howard being 52.6% Democrat, and Montgomery being 60.7% Democrat) -- or at least as variable as it can be in Maryland, a Blue state.
...
**Task** Run the following code to load the Zip Code Tabulation Area data for Maryland. What's the datatype of the ZCTA5N column? **Answer** The datatype of ZCTA5N is identified as a double (number).
