...
**Answer** To distinguish the result to be a new dataset that's different than the original one. It kind of reminds me of copying and editing a spreadsheet.
...
```{r}
# take maryland_expenses data frame, change it and save it
maryland_expenses <- maryland_expenses |>
# mutate expenditure_date into date data type
  mutate(
# set expediture_date date order to mdy
...
**Answer:** expenditure_date is now registered as a date, and it's reformatted by year-month-day.
...
```{r}
# take maryland_expenses
maryland_expenses |>
# creates "month" column, which is assigned to floor_date, which takes expediture_date column and extracts the month from that column
  mutate(
    month = floor_date(expenditure_date, "month")
  ) |>
# group data by month
  group_by(month) |>
# count up total spent by month
  summarise(
    total_amount = sum(amount)
  ) |>
# arrange table by total_amount in descending order
...
**Answer** All the dates listed are the first of each month.
...
```{r}
# saves changes to data frame
general_22 <- general_22 |>
# create two columns
  mutate(
# column counts total votes
    total_votes = cox + moore + lashar + wallace + write_ins,
# column divides moore's votes out of total_votes
...
**Answer** Two columns are created and saved to general_22 -- **total_votes,** which is a column that counts the total votes per county, and **pct_moore**, which creates divides Moore's votes out of total_votes per county.
...
**Answer** This code arranges general_22 in descending order by pct_moore, (the percentage of Moore's votes per county). It shows that a majority of voters in Prince George's County, Baltimore City, and Montgomery County voted for Moore.
...
**Answer** This is also interesting, because it's arranged by the counties who voted for Moore the least.
...
-   str_to_lower() converts the string to all lowercase

-   str_to_sentence() capitalizes the first character in a string
...
**Answer** The code takes maryland_expenses and makes a copy of the dataset. Then, it creates a new column called "state," that goes through each row and if it detects a specific state from the "address" column, it'll output the abbreviation of that state. If a listed state is not there, it'll output NA.
...
# if it's not one of the strings listed, set to NA
        .default = NA
      )
  )
```
...
**Answer** It does make sense to me, but I think I would need more context to better understand these numbers. This output lists the total amount of money spent per state, in descending order. But it's all just big numbers to me. I think I'd include a percentage column, to better compare across states.
...
1.  Which professor teaches the most classes per department, in a single academic school year?

2.  Which department has the most classes with the fewest seats?

3.  I'd be interested in pulling from the title or course descriptions, to see how many classes are provided that highlight key words of interest (e.g. climate change, film, etc.).
