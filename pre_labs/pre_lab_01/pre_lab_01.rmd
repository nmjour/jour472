...
In the space below, write the title of the course from the first row of the "title" column. **Answer:** Directed Research
...
```{r}
### it takes the data frame
umd_courses |>
### and then creates a bucket, sorts umd_courses by term. each row is a different term
  group_by(term) |> 
### and then it does math. in this case, counting each class per term
  summarize(
    count_classes = n()
  )
```
...
**Answer:** This is a table describing the total number of courses available per academic term.
...
Compared to the previous codeblock, what has changed and how did that affect the result? **Answer:** This is organized by the number of total courses in descending order. Meaning, the largest course listings per term are at the top.
...
**Answer:** This is organized by term in chronological order, and then for each term, the departments are listed alphabetically. The number of classes counted are for each department, per term.
...
umd_courses |>
  group_by(term) |>
### you can do multiple math things in a summarize commmand
  summarise(
    count_classes = n(),
...
Compared to the previous codeblock, what has changed and how did that affect the result? **Answer:**  The table is grouped by term, then organized by the total number of seats, in descending order. It also lists the total number of seats per term.
...
Compared to the previous codeblock, what has changed and how did that affect the result? What do the top terms have in common? **Answer** This table is also organized by the total number of seats, in descending order. However, there are new columns listing the average and median amount of seats per term. The top terms listed -- meaning, those that have the most number of seats -- are fall terms.
...
**Answer** Classes are grouped by department (in alphabetical order) instead of term. The minimum and maximum seats per department are also listed.
...
1.  Which department has grown or shrunk the most in course offerings over the years?

2.  Which departments or classes are more reliable in providing syllabi?

3.  Is the number of seats proportional to student demand? (Would have to compare seat numbers to enrollment numbers)
