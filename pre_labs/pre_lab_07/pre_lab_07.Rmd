...
```{r echo=FALSE}
census_api_key("15ea1e3cbece435f103f3ccdc9faa70c6c52107f", install=TRUE)
```
...
**Task** Run the following codeblock to calculate the percentage change and put the state with the largest percentage growth below. **Answer** The state whose population grew the largest from 2000 to 2010 was Nevada. The state's population grew 35% in that decade.
...
**Task** Run the following codeblock to find the Maryland county with the highest median household income and put the answer below. **Answer** The Maryland county with the highest median household income is Howard County, which is roughly \$129,549.
...
**Task** Run the following codeblock to create a chart showing the margin of error for each county. The current headline isn't very descriptive of the data. What should it be? **Answer** Estimated household income of Maryland counties, from 2017-2021 American Community Survey
...
Is the difference between Calvert and Montgomery significant? **Answer** No, the difference between Calvert and Montgomery isn't significant, as the bars for each county estimate overlaps.

Is the difference between Howard and everyone else significant? **Answer** Yes, the difference between Howard and everyone else is significant, as the ACS estimate and margin of error does not overlap with the estimate of other counties.
...
**Task** Let's get ZCTA household income data from the ACS using tidycensus for each zip code in `md_high_schools`. Run the following codeblock to retrieve household income data from 2019 - the latest year for which we have it for all Maryland ZCTAs - first, then join them to `md_high_schools` using the GEOID column. We'll also create min and max values and drop any rows where we don't have income estimates. What's one question we could ask of this data now that we have enrollment percentages and household income for the high school's zip code? What might make the answer harder to interpret? **Answer** Is there a correlation between household income levels and college enrollment percentages? More specifically, one question to ask is: are there higher college enrollment percentages in high schools that have high median household incomes?

Household income may not be entirely representative of a high school's student body, as some schools are magnet programs and/or have folks who live outside of that zip code and travel to attend such schools. School redistricting may also alter the relationship between the wealth of a school's neighborhood and its student body.
...
**Task** Let's look at the average household income for high schools that have 60% or more graduates immediately enrolling in college, and then look at those with less than 40%. Write a sentence describing the results. **Answer** In Maryland, there are 50 high schools that have a majority of graduates enrolled in college. These high schools have an average household income of \$111,448. On the otherhand, there are 76 Maryland high schools who do not have less than 40% of graduates enrolled in college, and those schools have an average household income of \$69,252.
...
**Task** What is another question you could ask of this data? **Answer** Do high schools on the Eastern Shore have a high amount of graduates enrolling in college? What is the average household income of this area?
