...
**Task** Run the following code and describe the dataframe it outputs. **Answer** The dataframe has the date, title, URL and text of Sen. Ben Cardin's press releases from 2021 to April 2023.
...
for (u in urls){
  ...
  # add row to container dataframe
...
**Task** Run the following code and describe what the `unique` function does to the original list of words. **Answer** The `unique` function only outputs unique or single instances of values. It is case sensitive (e.g. making "Dog" unique from "dog").
...
**Task** Run the following code and describe the output of using the `unnest_tokens` function. **Answer** From analyzing Sen. Cardin's press releases, there are over 647,000 unique words.
...
**Task** Run the following code and describe what the resulting graphic shows. Is it interesting? **Answer** The bar graph shows that "the" is the most used word in Sen. Cardin's press releases, but this is not interesting, as it doesn't really show the issues Sen. Cardin cares about/shares with his constituency. The graph is mostly flooded with other articles, too.
...
**Task** Run the following code and describe the results. Is it more interesting than before? **Answer** This list of unique words is much more interesting, as they give a better idea of what Sen. Cardin cares about. Most used words include "health," "support" and "project." Other important topics include healthcare, Baltimore, community work, funding and infrastructure.

```{r}
unique_words |>
# if these words match with stop_words, do not join them
...
In 2021, the top words counted revolved around COVID-19, public health and community.

In 2022, the top words counted involved funding, health and community. (In this list, "covid" is not mentioned directly.)

In 2023, other words related to legislation and human rights are more popular. Two words that stood out to me for this year are "women" (which likely got used more after Roe v. Wade was struck down after June 2022) and "fire."
...
**Task** Run the following code and describe the results. **Answer** In 2021, Sen. Cardin's press releases talked the most about the COVID-19 pandemic, public health, the American Rescue Plan Act (which was passed as a response to the economic impact of the pandemic) alongside key areas in Maryland, such as Baltimore city and the Chesapeake Bay.
...
**Task** Run the following code and describe the results. **Answer** In 2022, COVID-19 is still a top used phrase, but is no longer the number one, most mentioned phrase. The first two phrases involve community projects and funding amounts. After these top two phrases, the list is mostly places in Maryland -- Prince George's, Baltimore and the Chesapeake.
...
**Task** Run the following code and describe the results. **Answer** "Human rights" is the most used phrase, alongside other phrases about equity and rights. This list is more varied compared to previous years -- "oral health" and "offshore wind" are notable ones that stand out to me.
...
**Task** Run the following code and describe the results. Do any of the sentiments seem incorrect or counter-intuitive? **Answer** "Critical" in 2021 is determined to have a negative connotation, but the press releases might be using the word as a synonym for "important" or "necessary," which are more neutral words. "Cancer" in 2022 and 2023 is also listed as negative, but what if Sen. Cardin is talking about cancer research efforts?

The sentiment analysis looks at words individually and without context. For some words, such context is needed to better determine if words are used in a neutral, or in a emotional/sentimental manner.
