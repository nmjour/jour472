...
**Task** Run the following code to group and count records in the Grantee column, and filter to examine only recipients that start with W. Name a problem that you see that is preventing proper grouping. **Answer** There's a lot of duplicate names of grantees -- like the WATERFRONT PARTNERSHIP OF BALTIMORE, INC. also being listed as WATERFRONT PARTNERSHIP OF BALTO INC and WATERFRONT PARTNERSHIPS OF BALTIMORE.
...
- grantee_clean lists Washington County Museum of Fine Arts in all caps, compared to the sentence case of Grantee.

- The same is the case for the Tree House Child Advocacy Center. So, the results have been merged, but not standardized.
...
- WASHINGTON ADVENTIST Total	wasn't determined as similar enough to WASH ADVENTIST HEALTH SYSTEM or WASHINGTON ADVENTIST UNIVERSITY.

- Under grantee_clean, Washington Adventist University was turned into all uppercase.	

- I'm having trouble finding a third change.
...
**Task** Run the following code to use the n_gram_merge() function to clean the data, then group by grantee_clean and Grantee to examine recipients that start with W. Look at the differences. Does the changes it made vary substantially from the previous use of key_collision_merge? **Answer** No, there are no substantial changes.
