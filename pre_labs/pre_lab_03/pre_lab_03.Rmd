...
**Task** Run the following codeblock to reload the data, using every row to set the data types. Does it show any parsing errors when you run? Answer below **Answer** It shows no errors.
...
**Task** Run the following codeblock to glimpse the data. What data type is the "Vendor Name" field? What data type is the "Amount" field? What data type is the "Date" column? Answer below. **Answer** "Vendor Name" is a character, "Amount" is a double, "Date" is a character.
...
**Answer** 549 zip codes do not have five characters.
...
**Answer** Taking the data frame, we group it by fiscal year and category. Then we count each row (documented grant/loan) per category for each fiscal year. Then we sort the counted data by fiscal year. The output has random categories -- "L," "Contract." Some years have null categories.

```{r}
# take the data frame
md_grants_loans |> 
# group data by fiscal year, then by category
  group_by(`Fiscal Year`, Category) |> 
# count each grant/loan
  summarize(count = n()) |> 
# sort by fiscal year
  arrange(`Fiscal Year`)
```
...
**Answer** The smallest amount is a $60 grant to a land preservation trust.
...
**Answer** The column names are converted to lowercase and spaces in between words are replaced with underscores.
...
**Answer** We copy the data frame, then we clean the column names. Then, we rename the "grantor" column to "source." Then, we standardize the case of the "source," "grantee," and "description" columns. What is displayed is character data that is all uppercase.
...
**Answer** There are 50 possible duplicates.
...
**Answer** The new column on the far right lists 5-digit zip codes -- which comes from the first 5 digits (starting from the left side of each entry) from the "zip_code" column.
...
**Answer** The zip code that has received the most money from the Maryland Tourism Board is 21202, which is in downtown Baltimore and includes Inner Harbor. This result makes sense to me, as Inner Harbor is a huge tourist destination with many hotels and attractions.

```{r}
# copy data set
tourism_source <- cleaned_md_grants_loans |> 
# filter by source
  filter(
    str_detect(source, "MARYLAND TOURISM BOARD")
  ) |> 
# group by updated zip code
  group_by(
    zip5
  ) |> 
# add up total money received per zip
  summarize(
    total_amount = sum(amount)
  ) |> 
# arrange by counted instances
