...
**Task** Run the following code in the gray-colored codeblock below to extract all tables in the HTML and save it as a dataframe. What do you notice about the columns? **Answer** The column names (and data) are all in title case. The dates are also identified as a character data type, not a date data type.
...
**Task** Run the following code to extract the table we want, clean up the names and change the date column to an actual date. Look at the dataframe; what other information about these releases might you want to add? **Answer** I'd like to produce another dataframe that saves the text from each press release.
...
**Task** Run the following code in the gray-colored codeblock below to find the `<li>` tags. What does the output suggest about how many `<ul>` tags there are in the HTML source? **Answer** There are at least three unordered lists on the site. The function identifies all `<li>` tags on the page, which includes social media links and the menu items at the top of the website -- things we're not looking for.
...
**Task** Run the following code in the gray-colored codeblock below to find the text inside `<li>` tags. Describe the output: **Answer** The code takes the text from all of the html elements that are in `<li>` tags. The same issue from Task 9 persists -- text from social links and navigation menu items are combined with the press release list. But we can better see all the press release text.
...
**Task** Run the following code in the gray-colored codeblock below to make a dataframe. Describe the output: **Answer** Takingosp_results, the text values within `<li>` elements on the page are saved as a dataframe tibble titled releases.
...
releases <- osp_results |>
  ...
  html_text() |>
  as_tibble() |>
# take value item, separate into 2 columns, using : as separator
...
releases <- osp_results |> 
  ...
  as_tibble() |>
  separate(value, c('date', 'title'), sep=":") |>
# drop na from title column, which all happen to be meeting minutes
  drop_na(title) |>
# get rid of leading/trailing spaces in title column, update date column
