...
**Answer** Copy the url of the graphic you published here. It will begin with <https://www.datawrapper.de/>. Don't copy the browser's current url.

<https://www.datawrapper.de/_/0wprW/>
...
**Task** Look for "Hide regions without data" under Appearance, and click the slider icon to enable that feature. You should see a map zoomed into Maryland with some counties in various colors. **Answer** Instead of showing an entire map of all of the counties in the United States, the map changes to only focus on Maryland, broken down by county.
...
**Task** Click the "Customize tooltips" button so it expands down. Change {{ fips_code }} to {{ county }} and {{ pctmoore }} to {{ FORMAT(pctmoore, "00.0%")}} **Answer** When hovering over each county, the tooltip now says the county's name (instead for its FIPS code) and the percentage of votes with only 1 decimal point.
...
**Task** Click the "Proceed" button until you get to the "Publish & Embed" tab, then click "Publish Now". Copy the published URL (like you did for the chart above) and paste it below. **Answer** <https://www.datawrapper.de/_/lp42v/>
