...
**Task** Run the following code to load data. Describe what you see in the answer space below. What columns exist in this data? **Answer** The data is a shapefile. It has 8 columns of data, which includes Maryland zip codes, zip code area names, as well as shape length, shape area and geometry. From the `glimpse` function, R describes the data's geometry type as a multipolygon. The projection used is also Pseudo-Mercator.
...
**Task** Run the following code. Describe the output in the space below. **Answer** The code provides a light gray map of Maryland that outlines all of the zip codes in the state in black. It is plotted by latitude and longitude coordinates (which are the axes of the map).
...
**Task** Run the following code. Describe the output in the space below. **Answer** This code loads the tigris library.
...
**Task** Run the following code. Describe the output in the space below. **Answer** The code loads every county in the United States. The resulting data frame has the state and county FIPS identifiers, county names, GEOID, geometry and other identifiers.
...
**Task** Run the following code. Describe the output in the space below. **Answer** This code copies a new dataframe from the counties dataframe that only has the Maryland counties.
...
**Task** Run the following code. Describe the output in the space below. **Answer** This creates a map of Maryland counties, mapped by longitude and latitude coordinates. But the Chesapeake Bay isn't visualized as a state border.
...
**Task** Run the following code. Describe the output in the space below. **Answer** The output is a map of Maryland (with latitude and longitude coordinates) that has the outline of counties overlaid on top of the outline of zip codes. The county boundaries are larger than the zip code boundaries.
...
**Task** Run the following code. Describe the output in the space below. **Answer** This code loads foreclosure notice data by zip code in Maryland. The dataframe is then cleaned, then duplicated to only have the zip code column and foreclosure notice numbers in March 2023.
...
**Task** Run the following code. Describe the output in the space below. **Answer** A new dataframe is created that joins the Maryland zip code geometry data with the foreclosure notice data in March 2023. The dataframes are joined using their respective zip code columns.
...
**Task** Run the following code. Describe the output in the space below. **Answer** First, a centroid data frame is created, which provides the center point of each county in Maryland.

Second, the code outputs a map of Maryland that maps out foreclosure notices by zip code in March 2023. A color scale is used to fill each zip code, ranging from a dark blue (representing low numbers of foreclosure notices) to a sky blue (representing high numbers of foreclosure notices).

It seems like the highest rates of foreclosure notices in March 2023 were in zip codes in Prince George's County, Charles County, as well as in Baltimore City.
...
**Task** Run the following code. Describe the output in the space below. Be sure to input your census api key. **Answer** I loaded the tidyverse, sf, janitor and tidycensus libraries.
...
**Task** Run the following code. Describe the output in the space below. **Answer** A data frame that lists Maryland foreclosure notice data by county is loaded. Each county is its own column. The dataframe is sliced so only the first row of data (notices in March 2023) is kept. Then, the dataframe is transposed so that all the counties are now rows (grouped together in one column).

After, another dataframe is created that loads 2021 Maryland county population data from the Census.
...
**Task** Run the following code. Describe the output in the space below. **Answer A** data frame is created that loads 2021 Maryland county population data from the Census. It also loads the geometry data.
...
**Task** Run the following code. Describe the output in the space below. **Answer** We are able to view the county population dataframe and county notices dataframe side-by-side. The column names do not have consistent letter cases. The counties are also described in different ways.
...
**Task** Run the following code. Describe the output in the space below. **Answer** This code cleans up md_county_population. It renames the NAME column to county. It removes the " , Maryland" listed in each county data point, so it's consistent with the foreclosure data. The county names are also trimmed. The estimate column is renamed to population. Finally, the dataframe is condensed to only list the county, population and geometry columns.
...
**Task** Run the following code. Describe the output in the space below. **Answer** The cleaned up county population data frame is joined with the county foreclosure data frame. The data frames are joined by county name.
...
**Task** Run the following code. Describe the output in the space below. **Answer** A column is added to the joined dataframe that calculates the number of foreclosure notices per 1,000 people. The dataframe is then arranged from highest per capita rate to lowest per capita rate.
...
**Task** Run the following code. Describe the output in the space below. **Answer** The names of the counties are pulled from the county_centroids dataframe. A Maryland map is then created that maps out foreclosure rates per county. Text is added that lists the county names in the center of each jurisdiction. Some adjustments are made to make sure the county labels don't overlap.

From this visual, Charles County has the highest foreclosure rate in the state.
...
**Task** Run the following code. Describe the output in the space below. **Answer** A foreclosure notice map using the same data from the last task is created. The color scale changes color depending on the fill (which is the foreclosure rate number). Montgomery, Howard and Garrett have the lowest foreclosure notice rates, and they are filled in with black. Charles County remains the county with the highest foreclosure notice rate.
...
**Task** Run the following code. Describe the output in the space below. What regional patterns do you see? **Answer** The color scale is put on a logarithmic function, which makes the rate thresholds different per color. The differences are now more visually distinct.

More counties are now in black (which visualizes them as places that have the lowest foreclosure rates) --- these include Frederick, Talbot and Wicomico counties. There are also more counties that are orange, which represent counties that have high foreclosure rates, but not ones as high as Charles County.
