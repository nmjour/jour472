...
**Task** Run the following code to load the data. In the space below, indicate how many rows and columns it has. **Answer** The data set has 3822 rows and 4 columns.
...
**Task** Run the following code to add a month column to our dataframe using lubridate and count the number of calls for each month. In the space below, explain what each step of the code below is doing in English. Note the "label=TRUE" part in the mutate statement. **Answer** A new data frame is made from the Baltimore County 911 calls. Using `mutate`, a new column is created that pulls the month out of the date column for each record. The `label=TRUE` ensures that for each record, the month is counted as the abbreviated name of the month, and not the number of the month. Afterwards, the data frame is grouped by month, then calls are counted by month, then the data is arranged from highest number of calls per month to the lowest number of calls per month.
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer** It's blank -- just a grey square.
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer** The code produces a bar chart of the number of overdose calls documented per month, from January to December (chronologically).
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? **Answer** The bars are reordered from the least number of calls per month to the greatest number of calls per month. The x axis was also labeled to "reorder(month, total_calls)."
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? **Answer** The axes are flipped, with months being the y/vertical axis and the call count being the x/horizontal axis.
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? **Answer** A theme has been applied. The background is more transparent (has a grid with more negative space) and the data labels have smaller text.
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last version of the chart? **Answer** A different theme has been applied that has a light blue background, white lines and the data and axis titles are in black.
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? **Answer** The axis labels have been renamed to month and total call. There is also a title at the top for the entire chart (that describes its main finding) and a source tag at the bottom.
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer** This data set has the number of calls documented per date from February 6, 2022 to February 6, 2023.
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer** The code produces a line chart that visualizes the number of calls per day over the course of a year. Each spike/trough is a different day. The labels aren't descriptive enough.
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer** The x axis is reformatted to have a tick mark for every week. The start of every week is labeled by its month and day.
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer** The date labels have been rotated to a 45 degree angle. It's a bit more legible now.
